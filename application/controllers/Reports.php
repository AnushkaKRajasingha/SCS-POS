...
class Reports extends Secure_Controller
{
	...
	public function messagereport(){	
		...
		$config['smtp_user'] = 'scspossys@gmail.com';
		$config['smtp_pass'] = 'posAdmin@9';
		...
		$companyphone = $this->config->item('phone');		
		$this->email->from('scspossys@gmail.com','scspos admin');
		//$this->email->to($companyphone.'@mms.mobitel.lk');
		$this->email->to($companyphone.'@mms.dialog.lk');
		//$this->email->to('anushkakrajasingha@gmail.com');		
		if(!isset($_POST['data'])) exit;		
		$data = $_POST['data'];		
		//var_dump($data);
		
		$this->email->subject($data['subject'].' - '.$data['subtitle']);
		$title_arry = array_column($data['columns'],'title');
		$field_arry = array_column($data['columns'],'field');		
		$title_arry_str = implode(" | ", $title_arry);		
	...
	/*	$message = '
		...
		$fields_arry = array_combine($field_arry, $title_arry);		
		$message = '---------
				';
		
		foreach ($data['data'] as $values) {
			foreach ($fields_arry as $key => $value) {
				$message .= $value .':'.$values[$key].'
						';
			}
			$message .= '---------
				';
